  ...
  if (budgetId) {
    ...
    if (shouldRecurse) {
      ...
      if (previousBudgets.docs.length > 0) {
        ...
        // Re-anchor to UTC midnight using UTC date components (strips browser TZ offset)
        const prevPeriodStartUtc = new Date(Date.UTC(prevPeriodStart.getUTCFullYear(), prevPeriodStart.getUTCMonth(), prevPeriodStart.getUTCDate()))
        const prevPeriodEnd = new Date(prevBudget.endDate)
        prevPeriodEnd.setUTCHours(23, 59, 59, 999)
        const prevPeriodEndUtc = new Date(Date.UTC(prevPeriodEnd.getUTCFullYear(), prevPeriodEnd.getUTCMonth(), prevPeriodEnd.getUTCDate(), 23, 59, 59, 999))
        ...
        const prevTransactions = await payload.find({
          ...
          where: {
            and: [
              {
                date: {
                  greater_than_equal: prevPeriodStartUtc.toISOString(),
                },
              },
              {
                date: {
                  less_than_equal: prevPeriodEndUtc.toISOString(),
                },
              },
              {
                or: [
