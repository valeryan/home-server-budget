...
/** Normalize a date value to UTC midnight so browser-timezone offsets don't shift it by a day */
...
export const RecurringItems: CollectionConfig = {
  ...
  hooks: {
    beforeChange: [
      ({ data }) => {
        if (data.anchorDate) data.anchorDate = toUtcMidnight(data.anchorDate)
        return data
      },
    ],
  },
  fields: [
    {
      ...
      admin: {
        hidden: true,
        components: {
          Cell: '@/payload/admin/cells/RecurringItemCategoryCell#RecurringItemCategoryCell',
        },
      },
    },
    {
      ...
      fields: [
        {
          ...
          admin: {
            components: {
              Field: '@/payload/admin/fields/FieldsetLabel#RecurringScheduleLabel',
            },
          },
        },
        {
